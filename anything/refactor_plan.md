...
- [ ] `UserService.signUp()`의 이메일 중복가입 check-then-act(`findOneByEmail` 후 `save()`) 레이스 검토 — `User.email`엔 DB 유니크 제약이 있으나 동시 요청 시 유니크 위반으로 발생하는 `QueryFailedError`를 잡지 않아 의도한 409(`ConflictException`) 대신 500이 노출될 수 있음. participation 중복 참가 레이스 수정(2026-09-16) 중 code-review로 발견.
- [ ] `Challenge.title`/`Feed.title` 생성 시 동일 계열 중복 제목 레이스 보호 검토 — 제목엔 DB 유니크 제약이 있으나 동시 생성 시 유니크 위반 `QueryFailedError`를 잡아 409로 변환하는 로직이 없어 500이 노출될 수 있음. participation에 적용한 해결 패턴(`QueryFailedError` 캐치 + `driverError.sqlMessage`로 인덱스명 매칭 후 `ConflictException` 변환)을 재사용할지 검토. 같은 리뷰(2026-09-16)에서 발견.
