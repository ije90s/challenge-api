...
export class ParticipationController {
    ...
    @Post(":challengeId")
    async joinChallenge(@User() user: RequestUser, @Param("challengeId", ParseIntPipe) challengeId: number): Promise<ResponseParticipationDto>{
        return await this.participationService.create(user.id, challengeId);
    }

    @Get(":challengeId/giveup")
    async giveupChallenge(@User() user: RequestUser, @Param("challengeId", ParseIntPipe) challengeId: number): Promise<ResponseParticipationDto>{
        return await this.participationService.updateStatus(user.id, challengeId);
    }

    @Patch(":challengeId")
    async updateChallenge(@User() user: RequestUser, @Param("challengeId", ParseIntPipe) challengeId: number, @Body() dto: UpdateParticipationDto): Promise<ResponseParticipationDto>{
    ...
    @Get("mine")
    async getMyChallenge(@User() user: RequestUser, @Query() query: RequestQueryDTO): Promise<ResponsePagingDto<ResponseParticipationDto>>{
